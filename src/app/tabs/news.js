...
export const refresh_news = (news, date, latest_discovery, prestige) => {
    ...
    var rep_content = 'It is truly a rare occurance when a small company achieves major attention almost overnight, yet PLAYER has against the odds risen above its competitors, and seems poised to move onto the continental playing board. A steady stream of input, ideas and industry profit have all led to PLAYER\'s growing influence, yet only provided the foundation for its landmark breakthrough in academia, hailed by members of the Royal Society as "a force to truly behold in its potential power and impact".'
    ...
    if (prestige < LOW_PRESTIGE) {
        ...
        rep_content = 'Mockery, contempt and sadness were the most common views present in the annual review of PLAYER industry. Lack of profits, public and academic recognition and all-round disapproval regarding social strategy has left the company with little chance of rising out of the purported hole it seems to have dragged itself into.'
    } else if (prestige < MID_PRESTIGE) {
        rep_headline = "OVERALL DISAPPROVAL DESPITE RECOGNISING POTENTIAL IN SMALL FINDS"
        rep_content = 'Though one can see the positive findings, they unfortunately must try to overlook the more numerous flaws in the newly developed PLAYER. Major finds appear to have been avoided out of fear, and the minor victories have only served to propel rivals towards the true rewards waiting.'
    } else if (prestige < MID_HIGH_PRESTIGE) {
        rep_headline = "PLAYER REMAINS STANDING DESPITE VICIOUS COMPETITION"
        rep_content = "Everyone knows the cruelty present in modern industry, and perhaps none better than PLAYER. Yet despite the beating many claim it has taken, the modestly made company still remains in play. Academics consider its longevity is due to its helpful contributions to society and advancements in science, though the major finding is currently a source of intense competition between it and several other companies."
    }
